...
Use this as a quick map: **situation → use this from core**. No parameter details; see the library sources for those.
...
**Do not:** Implement dial, clipboard, maps, URL open, share, or location with custom/platform code in the app.
...
**Do not:** Reimplement “write image to file” or “add location to EXIF” in the app. After camera capture, use `PhotoSaveUtils` (and optionally `addLocationExif` then `savePhoto`).
...
**Do not:** Implement a separate camera capture/preview/recording stack when these APIs are available.
...
**Do not:** Reimplement infinite-scroll state and list-with-retry UI when `PaginatedFlowLoader` + `PaginatedListView` fit.
...
When vibecoding, match the current task to a row above and use the indicated core API rather than generating equivalent logic in the app.
